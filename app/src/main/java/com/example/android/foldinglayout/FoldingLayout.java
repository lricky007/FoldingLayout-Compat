...
public class FoldingLayout extends FrameLayout {
    ...
    private final static Canvas sHackCanvas = new Canvas();
    ...
    private void updateFold() {
        if (getMeasuredHeight() <= 0 || getMeasuredWidth() <= 0) {
            // we haven't measured our view yet
            return;
        }
    ...
    private void prepareCanvasJBMR2(int w, int h) {
        if (mFullBitmap == null || mFullBitmap.isRecycled() || (mFullBitmap.getWidth() != w || mFullBitmap.getHeight() != h)) {
            if (mFullBitmap != null && !mFullBitmap.isRecycled()) {
                mFullBitmap.recycle();
            }
            mFullBitmap = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888);
            sHackCanvas.setBitmap(mFullBitmap);
        }
    }
    ...
    */
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (IS_JBMR2) {
            super.dispatchDraw(sHackCanvas);
        }
